...
func newTestTranslatorWithTypes(typeMap map[string]types.RegoTypeDef) *Translator {
	return &Translator{
		TypeTrans: NewTypeDefs(&types.TypeAnalyzer{
			Types: typeMap,
			Refs:  map[string]ast.Value{},
		}),
		VarMap: make(map[string]string),
...
func TestRefToSmt_InputSchemaPath(t *testing.T) {
	...
	tr.TypeTrans.TypeInfo.Types["input.parameters.foo"] = types.NewObjectType(map[string]types.RegoTypeDef{
...
func TestRefToSmt_InputDataReviewNestedObject(t *testing.T) {
	...
	tr.TypeTrans.TypeInfo.Types["input.data.review.foo"] = types.NewObjectType(map[string]types.RegoTypeDef{
		"bar": types.NewObjectType(map[string]types.RegoTypeDef{
			"baz": types.NewAtomicType(types.AtomicString),
		}),
	})
...
func TestTermToSmt_BasicTypes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := tr.termToSmt(tt.term, context)
...
func TestExprToSmt_BasicOps(t *testing.T) {
	...
	got, err := tr.exprToSmt(&plusExpr, context)
	if err != nil {
	...
	got, err = tr.exprToSmt(eqExpr, context)
	if err != nil {
		t.Errorf("exprToSmt() error = %v", err)
	}
	want = "(= \"foo\" \"foo\")"
...
func TestExprToSmt_Advanced(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			context := NewTransContext()
			got, err := tr.exprToSmt(&tt.expr, context)
			if (err != nil) != tt.wantErr {
...
func TestExplicitArrayToSmt_Success(t *testing.T) {
	...
	got, err := tr.explicitArrayToSmt(arr, context)
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
	tr.AddTransContext(context)
	if got == "" {
...
func TestExplicitArrayToSmt_TypeNotFound(t *testing.T) {
	...
	_, err := tr.explicitArrayToSmt(arr, context)
	if err == nil {
...
func TestExprToSmt_AllCases(t *testing.T) {
	...
	t.Run("Builtins", func(t *testing.T) {
		...
		smt, err := tr.exprToSmt(expr, context)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		expected := "(+ 1 2)"
	...
	t.Run("TypeError", func(t *testing.T) {
		...
		_, err := tr.exprToSmt(expr, context)
		if err == nil {
			t.Errorf("expected error for missing type, got nil")
		}
	})
...
func TestExplicitArrayToSmt_CompareFullSmtLib(t *testing.T) {
	...
	varName, err := tr.explicitArrayToSmt(arr, context)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	tr.AddTransContext(context)
...
func TestHandleConstObject_AllCases(t *testing.T) {
	...
	t.Run("SimpleObject", func(t *testing.T) {
		...
		tr := newTestTranslatorWithTypes(typeMap)
		context := NewTransContext()
		varName, err := tr.handleConstObject(obj, context)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		tr.AddTransContext(context)
	...
	t.Run("TypeNotFound", func(t *testing.T) {
		...
		tr := newDummyTranslator()
		context := NewTransContext()
		_, err := tr.handleConstObject(obj, context)
		if err == nil {
			t.Fatalf("expected error for missing type info, got nil")
		}
	})
